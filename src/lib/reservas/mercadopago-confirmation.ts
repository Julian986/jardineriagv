import { ObjectId } from "mongodb";
import { getDb } from "@/lib/mongodb";
import { mpGetPayment } from "@/lib/mercadopago/api";
import type { MpPaymentResource } from "@/lib/mercadopago/types";
import { getReservaVisitaMontoArs } from "@/lib/mercadopago/config";

export type PaymentConfirmationOutcome =
  | "confirmed"
  | "already_confirmed"
  | "not_approved"
  | "missing_external_reference"
  | "turno_not_found"
  | "amount_mismatch"
  | "wrong_state"
  | "fetch_error";

export function buildMpExternalReference(turnoId: ObjectId): string {
  return `jgv_turno_${turnoId.toHexString()}`;
}

function expectedAmountArs(turno: { montoTotalVisitaArs?: number }): number {
  const n = Number(turno.montoTotalVisitaArs);
  return Number.isFinite(n) && n > 0 ? n : getReservaVisitaMontoArs();
}

export async function processApprovedPaymentForTurno(
  paymentId: string,
  auditPayload: { transport: string; rawSnippet: string },
): Promise<{ outcome: PaymentConfirmationOutcome; message?: string }> {
  const db = await getDb();
  const turnos = db.collection("turnos");
  const payments = db.collection("mp_payments");
  const audit = db.collection("mp_webhook_events");

  let payment: MpPaymentResource;
  try {
    payment = await mpGetPayment(paymentId);
  } catch (e) {
    const msg = e instanceof Error ? e.message : String(e);
    await audit.insertOne({
      createdAt: new Date(),
      paymentId,
      transport: auditPayload.transport,
      rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
      outcome: "fetch_error",
      detail: msg,
    });
    return { outcome: "fetch_error", message: msg };
  }

  const mpId = payment.id != null ? String(payment.id) : paymentId;
  const status = (payment.status ?? "").toLowerCase();

  await payments.updateOne(
    { mpPaymentId: mpId },
    {
      $set: {
        mpPaymentId: mpId,
        status: payment.status ?? null,
        statusDetail: payment.status_detail ?? null,
        externalReference: payment.external_reference ?? null,
        transactionAmount: payment.transaction_amount ?? null,
        currencyId: payment.currency_id ?? null,
        dateApproved: payment.date_approved ?? null,
        snapshot: payment as object,
        updatedAt: new Date(),
      },
      $setOnInsert: { createdAt: new Date() },
    },
    { upsert: true },
  );

  if (status !== "approved") {
    await audit.insertOne({
      createdAt: new Date(),
      paymentId: mpId,
      transport: auditPayload.transport,
      rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
      outcome: "not_approved",
      mpStatus: payment.status ?? null,
    });
    return { outcome: "not_approved", message: payment.status ?? undefined };
  }

  const ext = payment.external_reference?.trim();
  if (!ext) {
    await audit.insertOne({
      createdAt: new Date(),
      paymentId: mpId,
      transport: auditPayload.transport,
      rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
      outcome: "missing_external_reference",
    });
    return { outcome: "missing_external_reference" };
  }

  const turno = await turnos.findOne({ mpExternalReference: ext });
  if (!turno) {
    await audit.insertOne({
      createdAt: new Date(),
      paymentId: mpId,
      transport: auditPayload.transport,
      rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
      outcome: "turno_not_found",
      externalReference: ext,
    });
    return { outcome: "turno_not_found" };
  }

  const turnoId = turno._id as ObjectId;
  const expected = expectedAmountArs(turno as { montoTotalVisitaArs?: number });
  const paid = Number(payment.transaction_amount);
  if (Number.isFinite(paid) && paid > 0 && Math.round(paid) !== Math.round(expected)) {
    await audit.insertOne({
      createdAt: new Date(),
      paymentId: mpId,
      transport: auditPayload.transport,
      rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
      outcome: "amount_mismatch_logged",
      turnoId,
      expected,
      received: paid,
      detail: "Se confirma igual si el pago está approved (como marcelo_ponzio).",
    });
  }

  if (turno.estado === "confirmed") {
    const existing = turno.confirmedMpPaymentId != null ? String(turno.confirmedMpPaymentId) : "";
    if (existing === mpId) {
      await audit.insertOne({
        createdAt: new Date(),
        paymentId: mpId,
        transport: auditPayload.transport,
        rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
        outcome: "already_confirmed",
        turnoId,
      });
      return { outcome: "already_confirmed" };
    }
    await audit.insertOne({
      createdAt: new Date(),
      paymentId: mpId,
      transport: auditPayload.transport,
      rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
      outcome: "wrong_state",
      turnoId,
      detail: "confirmed_with_different_payment",
    });
    return { outcome: "wrong_state", message: "confirmed_with_different_payment" };
  }

  if (turno.estado !== "pending_payment") {
    await audit.insertOne({
      createdAt: new Date(),
      paymentId: mpId,
      transport: auditPayload.transport,
      rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
      outcome: "wrong_state",
      turnoId,
      detail: String(turno.estado),
    });
    return { outcome: "wrong_state", message: String(turno.estado) };
  }

  const now = new Date();
  const updated = await turnos.findOneAndUpdate(
    {
      _id: turnoId,
      estado: "pending_payment",
      mpExternalReference: ext,
    },
    {
      $set: {
        estado: "confirmed",
        mpLastPaymentId: mpId,
        mpPaymentStatus: payment.status ?? "approved",
        mpPaymentStatusDetail: payment.status_detail ?? null,
        confirmedMpPaymentId: mpId,
        mpPaidAt: now,
        updatedAt: now,
      },
    },
    { returnDocument: "after" },
  );

  if (!updated) {
    const again = await turnos.findOne({ _id: turnoId });
    if (
      again?.estado === "confirmed" &&
      again.confirmedMpPaymentId != null &&
      String(again.confirmedMpPaymentId) === mpId
    ) {
      await audit.insertOne({
        createdAt: new Date(),
        paymentId: mpId,
        transport: auditPayload.transport,
        rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
        outcome: "already_confirmed",
        turnoId,
      });
      return { outcome: "already_confirmed" };
    }
    await audit.insertOne({
      createdAt: new Date(),
      paymentId: mpId,
      transport: auditPayload.transport,
      rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
      outcome: "wrong_state",
      turnoId,
      detail: "race_or_state_changed",
    });
    return { outcome: "wrong_state", message: "race_or_state_changed" };
  }

  await audit.insertOne({
    createdAt: new Date(),
    paymentId: mpId,
    transport: auditPayload.transport,
    rawSnippet: auditPayload.rawSnippet.slice(0, 16_000),
    outcome: "confirmed",
    turnoId,
  });

  return { outcome: "confirmed" };
}

export function getPaymentExpiryDate(): Date {
  const raw = process.env.RESERVA_PAYMENT_EXPIRES_MINUTES?.trim();
  const mins = raw ? Number.parseInt(raw, 10) : 24 * 60;
  const safe = Number.isFinite(mins) && mins > 5 && mins < 10080 ? mins : 24 * 60;
  const d = new Date();
  d.setMinutes(d.getMinutes() + safe);
  return d;
}
